...
struct CustomizeModeSheet: View {
  ...
  init(
    ...
    shouldStartGame: Binding<Bool>
  ) {
    self.difficulty = difficulty
    _isPresented = isPresented
    _shouldStartGame = shouldStartGame
    // Initialize with default values, will be updated in onAppear
    _selectedDuration = State(initialValue: 30)
    _selectedMaxMistakes = State(initialValue: 3)
    _selectedRoundTimeout = State(initialValue: 1.5)
    _selectedNormalMaxMistakes = State(initialValue: 3)
    _selectedConfusionSpeed = State(initialValue: 1.8)
    _selectedHardMaxMistakes = State(initialValue: 3)
  }
                  ...
                  ForEach(MaxMistakes.allCases) { maxMistakes in
                    Button(action: {
                      ...
                      Text(maxMistakes.displayName)
                        .font(.system(size: 16, weight: .medium))
                        .foregroundColor(selectedMaxMistakes == maxMistakes
                          .rawValue ?
                          Color.purple :
                          Color.gray)
                  ...
                  ForEach(HardConfusionSpeed.allCases) { speed in
                    Button(action: {
                      ...
                      Text(speed.displayName)
                        .font(.system(size: 16, weight: .medium))
                        .foregroundColor(selectedConfusionSpeed == speed
                          .rawValue ?
                          Color.purple :
                          Color.gray)
                ...
                HStack(spacing: 0) {
                  ForEach(MaxMistakes.allCases) { maxMistakes in
                    Button(action: {
                      ...
                      Text(maxMistakes.displayName)
                        .font(.system(size: 16, weight: .medium))
                        .foregroundColor(selectedHardMaxMistakes == maxMistakes
                          .rawValue ?
                          Color.purple :
                          Color.gray)
                        .frame(width: 60, height: 36)
                  ...
                  ForEach(NormalRoundTimeout.allCases) { timeout in
                    Button(action: {
                      ...
                      Text(timeout.displayName)
                        .font(.system(size: 16, weight: .medium))
                        .foregroundColor(selectedRoundTimeout == timeout
                          .rawValue ?
                          Color.purple :
                          Color.gray)
                        .frame(width: 80, height: 36)
                ...
                HStack(spacing: 0) {
                  ForEach(MaxMistakes.allCases) { maxMistakes in
                    Button(action: {
                      ...
                      Text(maxMistakes.displayName)
                        .font(.system(size: 16, weight: .medium))
                        .foregroundColor(selectedNormalMaxMistakes ==
                          maxMistakes.rawValue ?
                          Color.purple :
                          Color.gray)
                        .frame(width: 60, height: 36)
                        .background(
  ...
  private func startGame() {
    ...
    if difficulty == .easy {
      customizationStore.updateEasySettings(
        duration: selectedDuration,
        maxMistakes: selectedMaxMistakes
      )
    } else if difficulty == .normal {
      customizationStore.updateNormalSettings(
        roundTimeout: selectedRoundTimeout,
        maxMistakes: selectedNormalMaxMistakes
      )
    } else if difficulty == .hard {
      customizationStore.updateHardSettings(
        confusionSpeed: selectedConfusionSpeed,
        maxMistakes: selectedHardMaxMistakes
      )
    }
