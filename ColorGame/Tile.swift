...
struct Tile {
  ...
  // Single truthy validator for Hard mode - used for both grid generation and
  // tap evaluation
  func isValidHard(announcedColor: Color) -> Bool {
    let announcedColorName = colorName(for: announcedColor)
    return backgroundColor != announcedColor && textLabel
      .uppercased() != announcedColorName.uppercased()
  }
...
extension Tile: Equatable {
  static func == (lhs: Tile, rhs: Tile) -> Bool {
    lhs.backgroundColor == rhs.backgroundColor && lhs.textLabel == rhs.textLabel
  }
}
