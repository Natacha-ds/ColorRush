...
struct GameView: View {
                ...
                GeometryReader { geometry in
                  ZStack(alignment: .leading) {
                    ...
                    RoundedRectangle(cornerRadius: 4)
                      ...
                      .frame(
                        width: geometry.size.width * (roundTimeRemaining / 1.5),
                        height: 8
                      )
                      .animation(
                        .linear(duration: 0.1),
            ...
            VStack(spacing: 20) {
              HStack(spacing: 20) {
                if selectedDifficulty == .hard {
                  HardModeTile(
                    tile: hardModeTiles.count > 0 ? hardModeTiles[0] : Tile(
                  ...
                  HardModeTile(
                    tile: hardModeTiles.count > 1 ? hardModeTiles[1] : Tile(
                      backgroundColor: .gray,
                      textLabel: "?"
                    ),
              ...
              HStack(spacing: 20) {
                if selectedDifficulty == .hard {
                  HardModeTile(
                    tile: hardModeTiles.count > 2 ? hardModeTiles[2] : Tile(
                      backgroundColor: .gray,
                      textLabel: "?"
                    ),
                  ...
                  HardModeTile(
                    tile: hardModeTiles.count > 3 ? hardModeTiles[3] : Tile(
                      backgroundColor: .gray,
                      textLabel: "?"
                    ),
  ...
  private func handleTileTap(_ index: Int) {
    ...
    // End round timer immediately when tile is tapped (prevents
    // double-counting)
    endRoundTimer()
    ...
    if selectedDifficulty == .hard {
      ...
      // Track tap patterns for Easy and Normal modes (prevent repetitive
      // tapping)
    ...
    if isCorrect {
      ...
      // Update best streak
      scoringLedger.bestStreak = max(
        scoringLedger.bestStreak,
        scoringLedger.currentStreak
      )
      ...
      print(
        "correct: \(tappedDescription) - Score: \(scoringLedger.finalScore)"
      )
    ...
    DispatchQueue.main.asyncAfter(deadline: .now() + 0.3) {
      ...
      if isGameSessionActive, !isGameOver {
        startNewRound()
      }
    }
  }
  ...
  private func trackTapPattern(index: Int) {
    ...
    print(
      "Tap tracking - Position \(index): \(tapCounts[index]) total, \(consecutiveTapsOnSamePosition) consecutive"
    )
  }
  ...
  private func startNewRound() {
    ...
    if selectedDifficulty == .hard {
      ...
      let tileDescriptions = hardModeTiles
        .map { "\($0.textLabel) on \(colorName(for: $0.backgroundColor))" }
  ...
  private func buildValidGrid() -> [Color] {
    ...
    if shouldPreventRepetitive {
      print(
        "Anti-repetitive tapping: Placing announced color at position \(targetPosition)"
      )
    }
  ...
  private func buildHardModeGrid() -> [Tile] {
    ...
    while attempts < maxAttempts {
      ...
      let backgroundMatchTile = Tile(
        ...
        textLabel: colorNames.first { $0 != colorName(for: announcedColor) }?
          .uppercased() ?? "BLUE"
      )
      ...
      // Fill remaining 2 slots with random tiles (background ≠ label
      // constraint)
      for _ in 0 ..< 2 {
        let backgroundColor = colorPalette.randomElement() ?? .red
        let availableLabels = colorNames
          .filter { $0 != colorName(for: backgroundColor) }
        let textLabel = availableLabels.randomElement() ?? "red"
        tiles.append(Tile(
          backgroundColor: backgroundColor,
          textLabel: textLabel.uppercased()
        ))
      }

      // Step C: Compute validCount using the single validator
      let validCount = tiles
        .filter { $0.isValidHard(announcedColor: announcedColor) }.count
  ...
  private func buildDeterministicHardModeGrid() -> [Tile] {
    ...
    let validColor = colorPalette
      .first { $0 != announcedColor && $0 != tiles[1].backgroundColor } ??
      .green
    let validLabel = colorNames
      .first {
        $0 != colorName(for: validColor) && $0 != colorName(for: announcedColor)
      } ?? "yellow"
    tiles.append(Tile(
      backgroundColor: validColor,
      textLabel: validLabel.uppercased()
    ))

    // Fourth tile: another valid tile or invalid tile to maintain 1-2 valid
    // count
    let fourthColor = colorPalette
      .first {
        $0 != announcedColor && $0 != validColor && $0 != tiles[1]
          .backgroundColor
      } ?? .yellow
    let fourthLabel = colorNames
      .first {
        $0 != colorName(for: fourthColor) && $0 !=
          colorName(for: announcedColor)
      } ?? "red"
    tiles.append(Tile(
      backgroundColor: fourthColor,
      textLabel: fourthLabel.uppercased()
    ))
  ...
  private func startGameSession() {
    ...
    if selectedDifficulty == .easy {
      ...
      confusionTimeRemaining = storedConfusionSpeed > 0 ? storedConfusionSpeed :
        1.8
      maxMistakes = storedMaxMistakes >= 0 ? storedMaxMistakes : 2
  ...
  private func endGameSession() {
    ...
    print(
      "Game session ended. Final score: \(scoringLedger.finalScore), Mistakes: \(mistakes)"
    )
  }
  ...
  private func startConfusionTimer() {
    ...
    confusionTimer = Timer
      .scheduledTimer(withTimeInterval: 0.1, repeats: true) { _ in
        confusionTimeRemaining -= 0.1
  ...
  private func handleConfusionTimeout() {
    // Only process confusion timeout if game is still active and not already
    // game over
    guard isGameActive, isGameSessionActive, !isGameOver else { return }
    ...
    if selectedDifficulty == .hard {
      ...
      let tileDescriptions = hardModeTiles
        .map { "\($0.textLabel) on \(colorName(for: $0.backgroundColor))" }
      print(
        "Confusion refresh - New Hard mode grid: [\(tileDescriptions.joined(separator: ", "))]"
      )
    ...
    DispatchQueue.main.asyncAfter(deadline: .now() + 0.1) {
      if isGameSessionActive, !isGameOver, isGameActive {
        startConfusionTimer()
      }
    }
  }
  ...
  private func endGameWithReason(_ reason: GameEndReason) {
    ...
    isNewBestScore = highScoreStore.updateBestScore(
      for: selectedDifficulty,
      score: scoringLedger.finalScore
    )

    // Save score to leaderboard (map Difficulty to MistakeTolerance)
    let mistakeTolerance: MistakeTolerance = switch selectedDifficulty {
    case .easy:
      .easy
    case .normal:
      .normal
    ...
    LeaderboardStore.shared.addScore(
      ...
      for: mistakeTolerance
    )

    endGameSession()

    print(
      "Game over: \(reason), Final score: \(scoringLedger.finalScore), Mistakes: \(mistakes), New best: \(isNewBestScore)"
    )
  }
...
struct GameOverView: View {
  ...
  init(
    ...
    emotionalMessage = messages.randomElement() ?? "Well done!"
  }
  ...
  private var gameOverReason: String {
    switch endReason {
    case .tooManyMistakes:
      "You reached the maximum mistakes allowed."
    case .scoreBelowZero:
      "Score fell below zero."
    default:
      "Game ended."
    }
  }
      ...
      ZStack {
        ...
        ForEach(0 ..< 8, id: \.self) { index in
          Circle()
            ...
            .frame(width: CGFloat
              .random(in: 12 ... 28)) // Reduced from 20-40 to 12-28
            .position(
